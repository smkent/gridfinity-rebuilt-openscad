...
module cut_custom() {
    ...
    for (mm = [0:1:1])
    mirror([mm, 0, 0])
    cut(0, 0, xx, gridy, 5, 2.0);

    intersection() {
        cut(xx, 0, (gridx - xx * 2), gridy, 5, 2.0);
        #translate([0, 0, 25.2 - 8])
        if(0) {
            linear_extrude(height=8)
            square([16, gridy * 42 - 5.6], center=true);
        } else {
            rotate([0, 90, 0])
            ...
            union() {
                ...
                // square([8, yy], center=true);
                for (cy = [-6, -8])
                translate([cy, 0])
                translate([0, -yy / 2 + 8/2])
                circle(d=8);
    ...
    if(1) {
        ...
        // cylinder(h=24, d=8, center=true);
    }

    if(0)
    difference() {
        cut(0.8, 0, 0.4, gridy, 5, 2.0);
        if(0)
        translate([0, 0, 6.000])
        translate([0, 0, 4])
        rotate([-5, 0, 0])
        for (mz = [0:1:1])
        mirror([0, 0, mz])
        linear_extrude(height=mz ? 7 * gridz : 3)
        square([16, 130], center=true);
    }

    // cut(0, 0, nd_x, gridy, 5, 2.0);

    // cut(nd_x, 0, wr_x, gridy - 0.5, 5, 2.0);

    // cut(nd_x + wr_x, 2, (gridx) * 0.7, 1, 5, 0);
    // cut(nd_x + wr_x, 0, (gridx) - (nd_x + wr_x), gridy, 5, 2.0);
}
